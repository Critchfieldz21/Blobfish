...
namespace BackendLibrary
{
    public class Detect
    {
        ...
        public static (int pageNumber, float boxX, float boxY, float boxWidth, float boxHeight) GetRectInfo(string modelPath, string filePath)
        {
            System.IO.Directory.CreateDirectory(".\\temp\\");
            return ProcessImage(modelPath, filePath, ".\\temp\\", 72, 72);
        }

        public static (int pageNumber, float boxX, float boxY, float boxWidth, float boxHeight) GetRectInfo(string modelPath, string filePath, Stream pdffile)
        {
            System.IO.Directory.CreateDirectory(".\\temp\\");
            return ProcessImage(modelPath, filePath, pdffile, ".\\temp\\", 72, 72);
        }

        public static (int pageNumber, float boxX, float boxY, float boxWidth, float boxHeight) ProcessImage(string modelPath, string imagePath, Stream pdffile, string outputFolder, float dpiX = 72, float dpiY = 72)
        {
            ...
            try
            {
                ...
                var detectedBoundingBox = Detect.Detection(modelPath, savedFilePath, savedFilePath);
        ...
        public static (int pageNumber, float boxX, float boxY, float boxWidth, float boxHeight) ProcessImage(string modelPath, string imagePath, string outputFolder, float dpiX = 72, float dpiY = 72)
        {
            string input = Path.GetFileName(imagePath);
            string pattern = @"(?:_P)(\d+)";
            Match match = Regex.Match(input, pattern);
        ...
        private static SKRectI Detection(string modelPath, string filePath, string outputPath)
        {
